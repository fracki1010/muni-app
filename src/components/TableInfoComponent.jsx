...
export const TableInfoComponent = ({
  ...
  const renderCell = React.useCallback((movement, columnKey) => {
    const cellValue = movement[columnKey];

    switch (columnKey) {
      case "typeMovement":
        return cellValue === "input" ? (
          <Tooltip content="Entrada">
            <ArrowCircleDown color="success" />
          </Tooltip>
        ) : cellValue === "output" ? (
          <Tooltip content="Salida">
            <ArrowCircleUp color="error" />
          </Tooltip>
        ) : (
          <Tooltip content="Actualizado">
            <Edit fontSize="small" />
          </Tooltip>
        );
      case "date":
        return (
          <p className="text-bold capitalize">
            {formatShortDate(cellValue, false)}
          </p>
        );
      case "quantity":
        return (
  ...
  return (
    ...
    >
      ...
      <TableBody items={movements} emptyContent={<Spinner variant="wave" />} >
        {(item) => (
          <TableRow
            key={item.id}
            // className={`${
